...
CONFIG_SCHEMA = {
    ...
    "log_child_diagnostics":        {"type": "bool",  "default": True,        "label": "输出子策略诊断摘要（验证各引擎是否有效执行）"},
    "factor_style_single_engine_min_score": {"type": "float", "default": 86.0, "label": "因子类单引擎更高门槛（v6.2: 默认86）"},
    "factor_style_single_engine_require_confirmation": {"type": "bool", "default": True, "label": "因子类单引擎需H1启动/突破确认"},
    "max_factor_style_single_engine_per_side": {"type": "int", "default": 1,   "label": "每个方向最多保留几个因子类单引擎信号"},
...
class AICrossSectionDualFactorComboScanner(BaseScannerStrategy if _HAS_SCANNER_BASE else object):
    ...
    def scan_all_symbols(self, symbols: List) -> Dict[str, Any]:
        ...
        if bool(self.config.get("log_child_diagnostics", True)):
            self._log_child_diagnostics(by_strategy, raw_counts, timing)
        ...
        for item in output:
            ...
            if is_consensus:
                ...
                source_strategy = str(item.get("source_strategy", ""))
                source_style = self._get_engine_style(source_strategy)
                if source_style == "factor":
                    eff_single = max(
                        eff_single,
                        float(self.config.get("factor_style_single_engine_min_score", 86.0) or 86.0)
                    )
                ...
                if (
                    source_style == "factor"
                    and bool(self.config.get("factor_style_single_engine_require_confirmation", True))
                    and not has_starter
                ):
                    is_bo, bo_conf, _ = self._h1_breakout_detect(sym_obj)
                    if not (is_bo and bo_conf >= 0.67):
                        logger.info(f"[因子单引擎确认] 淘汰 {sym_id}: {source_strategy} 缺少H1启动/突破确认")
                        continue
                ...
                if item.get("passed"):
                    filtered.append(item)
        filtered = self._cap_factor_style_single_engine_results(filtered)
    ...
    def _log_child_diagnostics(self, by_strategy: Dict[str, List[Dict[str, Any]]],
                               raw_counts: Dict[str, int],
                               timing: Dict[str, float]) -> None:
        """输出每个子策略的产出数量、耗时和Top样本，验证是否真的在执行。"""
        parts = []
        for name, _, _ in self.child_strategies:
            kept = len(by_strategy.get(name, []))
            raw = int(raw_counts.get(name, kept))
            cost = float(timing.get(name, 0.0))
            top_items = by_strategy.get(name, [])[:2]
            if top_items:
                top_desc = ", ".join(
                    f"{it.get('symbol','?')}/{it.get('direction','WAIT')}({float(it.get('score', 0) or 0):.1f})"
                    for it in top_items
                )
            else:
                top_desc = "无信号"
            parts.append(f"{name}: 原始{raw} 保留{kept} 耗时{cost:.1f}s Top[{top_desc}]")
        if parts:
            logger.info("[五引擎子策略诊断] " + " | ".join(parts))

    def _cap_factor_style_single_engine_results(self, items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """限制因子类单引擎结果的数量，避免截面类信号淹没其他风格。"""
        limit = int(self.config.get("max_factor_style_single_engine_per_side", 1) or 1)
        if limit <= 0:
            limit = 0
        kept: List[Dict[str, Any]] = []
        factor_counts = {"BUY": 0, "SELL": 0}
        for item in items:
            is_consensus = "共振" in str(item.get("category", ""))
            if is_consensus:
                kept.append(item)
                continue
            style = self._get_engine_style(str(item.get("source_strategy", "")))
            direction = str(item.get("direction", "WAIT")).upper()
            if style == "factor" and direction in {"BUY", "SELL"}:
                if factor_counts[direction] >= limit:
                    logger.info(f"[因子单引擎限流] 淘汰 {item.get('symbol','?')}: {direction} 超过每方向上限 {limit}")
                    continue
                factor_counts[direction] += 1
            kept.append(item)
        return kept
